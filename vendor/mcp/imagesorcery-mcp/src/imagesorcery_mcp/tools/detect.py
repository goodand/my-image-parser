...
def register_tool(mcp: FastMCP):
    ...
    def detect(
        ...
        try:
            # Prefer a preconfigured writable cache directory from the launcher.
            yolo_config_dir = os.environ.get("YOLO_CONFIG_DIR", str(Path("models").absolute()))
            ...
            with redirect_stdout(sys.stderr):
                results = model(input_path, conf=confidence, verbose=False)[0]
