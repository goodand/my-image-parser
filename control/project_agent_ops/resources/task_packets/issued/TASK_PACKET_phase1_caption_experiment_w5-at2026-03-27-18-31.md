# Task Packet: Phase 1 Caption Experiment W5

## Goal

Run the phase-1 extracted-media OpenAI caption experiment for shard `W5` only, with the shard input and worker ledger fixed in advance.
...
- required sidecars derived from the ledger stem:
  - `control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w5_responses/`
  - `control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w5_execution_records.jsonl`
  - `control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w5_evaluation_decisions.jsonl`
...
1. Process only shard `W5`.
2. Use exactly the shard JSONL and ledger output fixed in `Fixed Contract`.
3. Keep the write set bounded to the ledger stem for `phase1_ppt2_w5` only.
...
- The run only covers the rows present in shard `W5`.
- The completion summary reports processed count, failed count, and the exact output paths used.

## Stop Conditions

- the shard JSONL is missing, unreadable, or does not resolve to `phase1_ppt2_w5.jsonl`
...
python3 scripts/caption_images_openai.py \
  --dataset-jsonl control/project_domain/resources/manifests/phase1_caption_shards/phase1_ppt2_w5.jsonl \
  --output control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w5.json \
  --model gpt-4.1 \
  --detail high
```
...
This packet is immutable for shard `W5`. If scope, ownership, or output paths change, publish a new issued packet instead of widening this one.
