# Task Packet: Phase 1 Caption Experiment W4

## Goal

Run the phase-1 extracted-media OpenAI caption experiment for shard `W4` only, with one fixed shard-to-ledger binding and no writes outside this worker's output stem.
...
- required sidecars:
  - `control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w4_responses/`
  - `control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w4_execution_records.jsonl`
  - `control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w4_evaluation_decisions.jsonl`
- shard facts:
  - `12` rows
  - first `image_id` in shard order: `02_1:image20.png`
  - last `image_id` in shard order: `02_1:image30.png`
...
python3 scripts/caption_images_openai.py \
  --dataset-jsonl control/project_domain/resources/manifests/phase1_caption_shards/phase1_ppt2_w4.jsonl \
  --output control/project_agent_ops/registry/jobs/image_caption_jobs/phase1_ppt2_w4.json \
  --model gpt-4.1 \
  --detail high
```
...
- The run covers only the `12` rows in shard `W4`.
...
This packet is immutable for shard `W4` and ledger stem `phase1_ppt2_w4`. If shard, ledger, model, or scope changes, publish a new packet instead of widening this one.
